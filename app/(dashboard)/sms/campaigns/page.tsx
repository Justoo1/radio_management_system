...
import { Search, Plus, Edit, Trash2, Send, MessageSquare, ArrowUpRight, Loader2, CreditCard, Users, BarChart3 } from 'lucide-react'
...
interface SMSCampaign {
  ...
  name: string
  description: string
  status: 'DRAFT' | 'SCHEDULED' | 'PROCESSING' | 'SENT' | 'PARTIALLY_SENT' | 'COMPLETED' | 'FAILED'
...
function SMSCampaignsContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch campaigns and credits in parallel
        const [campaignsRes, creditsRes] = await Promise.all([
          fetch('/api/sms/campaigns?pageSize=100'),
          fetch('/api/sms/credits'),
        ])
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'PROCESSING':
        return 'bg-purple-500/30 text-purple-300 border border-purple-500/50 animate-pulse'
      case 'DRAFT':
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCampaigns(campaigns.map(c =>
          c.id === campaignId ? { ...c, status: 'PROCESSING' as const } : c
        ))
      } else {
        ...
        <div className="mb-8">
          <div className="flex items-start justify-between mb-6">
            ...
            <div className="flex gap-3">
              <Link
                href="/sms/analytics"
                className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-4 py-3 rounded-xl font-medium border border-white/20 transition-all duration-300"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-slate-500 text-xs mt-1">
                      <span className="text-emerald-400 font-semibold">{sentCampaigns}</span> sent
                    </p>
                  </div>
            ...
            <div className="group relative">
              ...
              <div className="relative bg-white/10 backdrop-blur-xl rounded-xl p-6 border border-white/20 hover:border-cyan-500/50 transition-all duration-300">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-slate-400 text-sm font-medium mb-1">SMS Credits</p>
                    <p className="text-3xl font-bold text-white">
                      {credits ? credits.totalAvailable.toLocaleString() : '-'}
                    </p>
                    <p className="text-slate-500 text-xs mt-1">
                      {credits?.monthlyAllocation ? (
                        <>
                          <span className="text-cyan-400 font-semibold">{credits.monthlyRemaining.toLocaleString()}</span> monthly remaining
                        </>
                      ) : (
                        'Available balance'
                      )}
                  ...
                  <CreditCard className="w-8 h-8 text-cyan-400/50" />
                </div>
              </div>
            </div>

            {/* Contacts */}
            <Link href="/sms/contacts" className="group relative block">
              ...
              <div className="relative bg-white/10 backdrop-blur-xl rounded-xl p-6 border border-white/20 hover:border-purple-500/50 transition-all duration-300 h-full">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-slate-400 text-sm font-medium mb-1">Contacts</p>
                    <p className="text-3xl font-bold text-white">Manage</p>
                    <p className="text-slate-500 text-xs mt-1">
                      <span className="text-purple-400 font-semibold">Import & organize</span> contacts
                    </p>
                  </div>
                      ...
                      <tr key={campaign.id} className="hover:bg-white/5 transition-colors duration-200">
                        ...
                        <td className="px-6 py-4 text-sm">
                          {campaign.totalRecipients > 0 ? (
                            <div className="space-y-1">
                              <p className="text-white font-medium">{campaign.totalRecipients.toLocaleString()}</p>
                              {(campaign.status === 'SENT' || campaign.status === 'COMPLETED' || campaign.status === 'PARTIALLY_SENT') && (
                                <div className="flex gap-2 text-xs">
                                  <span className="text-emerald-400">{campaign.deliveredCount} delivered</span>
                        ...
                        <td className="px-6 py-4 text-sm">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold backdrop-blur ${getStatusColor(campaign.status)}`}>
                            {campaign.status === 'PROCESSING' && <Loader2 className="w-3 h-3 mr-1 animate-spin" />}
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {(campaign.status === 'DRAFT' || campaign.status === 'SCHEDULED') && (
                              <button
                                ...
                                className="p-2 text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/20 rounded-lg transition-all duration-200 disabled:opacity-50"
                                title="Send"
                              >
