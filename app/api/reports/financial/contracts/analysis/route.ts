...
export async function GET(request: Request) {
  try {
    ...
    // Check contract analysis report feature access
    const { hasAccess } = await checkFeatureAccess(prisma, user.organizationId, Feature.REPORT_CONTRACTS)
    if (!hasAccess) {
      return NextResponse.json(
        { error: 'Contract analysis report is not enabled for your organization. Please upgrade your plan.' },
        { status: 403 }
      )
    }
